...
#------------------------------------------------------------------------------- 
  ...
  parafunc_foreach=function(iu,p,q,m,tau,nn_size){
    #test location
    #Yhat=c()#if you run locally you can use this forloop as it failes on personal laptop because of cpu
    #for(iu in 1:100){
    ...
    k_NN=train_QQ[order(dst)[1:nn_size],]
#------------------------------------------------------------------------------------
    #model  the effect of l local points
    model = EzGP_fit((k_NN[,1:(p+q)]),k_NN[,(p+q+1)],p=p,q=q,m=m,tau=tau)  
    pred= EzGP_predict(test_QQ[iu,1:(p+ q)], model, MSE_on = 0)
    Yhat= pred$Y_hat#save
    #Yhat[iu]= pred$Y_hat#save
    #}
...
#parallel process    
  ...
  results <- foreach(iu=icount(nr), .combine='rbind',
                     .packages=c('nloptr', "laGP","EzGP")) %dopar% {
                       parafunc_foreach(iu=iu,p=p,q=q,m=m,tau=tau,nn_size=nn_size)
                     }
