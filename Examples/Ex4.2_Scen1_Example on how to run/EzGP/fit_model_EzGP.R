...
##EZGP added
#' @export
fit_model_EzGP <- function(y, locs, X = NULL, covfun_name = "EzGP_cov",
    NNarray = NULL, start_parms = NULL, reorder = TRUE, 
    ...
    for(i in 1:length(m_seq)){
        ...
        fit <- fisher_scoring( likfun,invlink(start_parms),
...
get_start_parms_modif <- function(y,X,locs,covfun_name,p_e,q_e,m_e){
    ...
    if(covfun_name ==("EzGP_cov")){
      npar=1+p_e+q_e+p_e*sum(m_e)

      start_parms <- c(rep(var(y),q_e+1))
        dmat <- fields::rdist(locs[randinds,])
        start_parms <- c(start_parms, rep(stats::median(dmat)/4,(npar-q_e-1)) )

      start_range <- median( dmat )/4
      start_parms <- c(rep(start_var,q_e+1), rep(start_range, (npar-q_e-1)))
    }
