...
updateloglike_CPP<- function(data, model, tau = 0, lb = "T", ub = "T",  x0 = "T",
                             xtol_rel = 1.0e-5, maxeval = 20, algorithm = "NLOPT_LD_LBFGS"){
  ...
  #  #A=Phi_gg
  #  Phi_gg=cov_EzGP_cpp(W1=X,W2=X, p=p, q=q, m=m, parv=solpar, tau = tau)
  #  #B=Phi_gl
  #  Phi_gl=cov_EzGP_cpp(W1=X,W2=x_ll, p=p, q=q, m=m, parv=solpar, tau = tau)
  #  #D=Phi_ll
  #  Phi_ll=cov_EzGP_cpp(W1=x_ll,W2=x_ll, p=p, q=q, m=m, parv=solpar, tau = tau)
  # 
  # 
  #  #Phi_mm
  #  Phi_mm<- rbind(cbind(Phi_gg, Phi_gl),cbind(t(Phi_gl), Phi_ll))
