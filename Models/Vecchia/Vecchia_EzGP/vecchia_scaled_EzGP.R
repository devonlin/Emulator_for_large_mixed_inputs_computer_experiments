...
library(laGP)
#####   fitting function   ########
fit_scaled_EzGP=function(y,inputs,inputs_encode,ms=c(30),trend='intercept',X,scale='none',q_e,p_e,m_e,
              var.ini,ranges.ini.p,ranges.ini.q,cur.ranges.q,select=Inf,print.level=0,max.it=32,tol.dec=4,
  ...
  if(missing(var.ini)) {
    ...
    #cur.var=rep(50.5, (q_e+1))
    } else cur.var=var.ini
  
  ## default range parameters
  input.ranges.p=apply(matrix(inputs[,1:p_e],ncol=p_e),2,function(x) diff(range(x)))
  #cur.ranges.q=rep(5.05, length((p_e+1):(npar-q_e-1)))
  if(missing(ranges.ini.p)) cur.ranges.p=.2*input.ranges.p else cur.ranges.p=ranges.ini.p
  if(missing(ranges.ini.q)) cur.ranges.q=.2*rep(5.05, length((p_e+1):(npar-q_e-1))) else cur.ranges.q=ranges.ini.q
  #if(missing(ranges.ini.p)) cur.ranges.p=5.05*input.ranges.p else cur.ranges.p=ranges.ini.p
  cur.ranges=c(cur.ranges.p,cur.ranges.q)
  cur.ranges_full=sum(cur.ranges)
  ############################################################################################
  ## only use subsample for estimation?
  ###Instead use number of matches
  ############################################################################################
  if(n.est<n){
  ...
  for(i.m in 1:length(ms)){
    ...
    while(conv==FALSE & maxit<=max.it){
      ...
      if(scale=='parms'){ scales=1/cur.ranges.p
      } else if(scale=='none'){ scales=1
      }else if(scale=='add'){ scales=1/cur.ranges_full
      }else stop(paste0('invalid argument scale=',scale))

      
      ## order and condition based on current params
      inputs.scaled=cbind(t(t(inputs_encode[,1:p_e])*scales),inputs_encode[,(p_e+1):(ncol(train_QQ_encode)-1)])
      ...
      NNarray=find_ordered_nn(inputs.ord_nn,m)
      
      cur.parms=c(cur.var,cur.ranges.p,cur.ranges.q)
      
      ## fisher scoring
      fit=fit_model_EzGP(y.ord,inputs.ord,X.ord,
                     NNarray=NNarray,m_seq=m,convtol=tol,
                     start_parms=cur.parms,max_iter=maxit,
                     covfun_name="EzGP_cov",silent=(print.level<2),
                     reorder=FALSE,fixed_parms=NULL,p_e=p_e,q_e=q_e,m_e=m_h)
      cur.var=fit$covparms[1:(q_e+1)]
      cur.ranges.p=fit$covparms[(q_e+1)+(1:(p_e))]
      cur.ranges.q=fit$covparms[(q_e+1)+((p_e+1):(npar-q_e-1))]
      conv=fit$conv
      maxit=maxit*2
  ...
  fit$covparms=c(cur.var,cur.ranges.p,cur.ranges.q)
  fit$p_e=p_e
  fit$q_e=q_e
  fit$m_e=m_e
  fit$trend=trend
...
predictions_scaled_EzGP <- function(fit,locs_pred,locs_pred_encode,m=100,nsims=0,
  ...
  q_e=fit$q_e
  m_e=fit$m_e
  npar=1 + q_e + p_e + p_e*sum(m_e)
  
  # specify trend if missing
  if(missing(X_pred)){
    if(fit$trend=='intercept'){
      X_pred=as.matrix(rep(1,n_pred))
    } else stop('X_pred must be specified')
  }
  
  # specify how to scale input dimensions
  if(scale=='parms'){ scales=1/covparms[(q_e+1)+(1:(p_e))]
  } else if(scale=='add'){ scales=1/sum(covparms[(q_e+1+p_e):(q_e+1+p_e+p_e*prod(m_h))])
  }else if(scale=='none'){ scales=1
  }else stop(paste0('invalid argument scale=',scale))
  ...
  return(list(preds=preds,NN_pred=NN_pred))
...
fit_vcf=function(fit,m.pred=140,n.test=min(1e3,round(nrow(fit$locs)/5)),
  ...
  # make predictions
  preds=predictions_scaled_EzGP(fitsearch,locs_pred=fit$locs[inds.test,,drop=FALSE],
