        ...
        for (int i2 = 0; i2 <= i1; i2++) {
         ...
         // Rcpp::Rcout << "theta0: " << theta0 << ", thetah: " << thetah << std::endl;
            ...
            //for (int h = 0; h < q_e; ++h) {
             ...
             //   double sum_term = psum(x1, x2, thetah);
             //   res1 += sigmah[h] * exp(sum_term);
            //}
            std::vector<double> qual(q_e, 0.0);  // Assuming qual is a vector of length q

             for (int h = 0; h < q_e; ++h) {
             int qual_ind = (z1[h] != z2[h]) ? 0 : 1;
             qual[h] = sigmah[h] * qual_ind;
             }

            double sum_qual = std::accumulate(qual.begin(), qual.end(), 0.0);
            res1 += sum_qual * std::exp(psum(x1, x2, thetah));
            ...
            for (int h = 0; h < q_e; ++h) {
             ...
             double sum_qual = std::accumulate(qual.begin(), qual.end(), 0.0);
                for (int s = 0; s < p_e; s++) { 
                   ...
                   // int end_index = start_index + p_e - 1;
                    //arma::vec theta_segment = thetah.subvec(start_index, end_index);
                    dcovmat(i1, i2, 1 + q_e + p_e +  s) = -sum_qual* pow((x1[s] - x2[s]), 2) * exp(psum(x1, x2, thetah));
                }
...
{
    if (covfun_name_string.compare("HEzGP_cov") == 0) { 
        p_covfun[0] = HEzGP_cov;
        p_d_covfun[0] = d_HEzGP;
    } else { 
