...
#-------------------------------------------------------------------------------
  ...
  fit.sv=fit_scaled_HEzGP2(y=train_QQ[,1+p_e+q_e],inputs=train_QQ[,1:(p_e+q_e)],
                          ...
                          ms=ms[1],n.est=min(5e3,train_size*prod(m_h)),p_e=p_e,q_e=q_e,m_e=m_h,scale="HE")
  #predict SVecchia
  
    Yhat_each[,l]=predictions_scaled_HEzGP2(fit=fit.sv,locs_pred=test_QQ[,1:(p_e+q_e)],locs_pred_encode=test_QQ_encode[,1:(ncol(test_QQ_encode)-1)],m=nn_size,scale = "HE")$means
