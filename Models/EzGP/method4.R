...
parafunc_foreach=function(iu,p,q,m,tau,ns_twin){
  # qualitative inputs
   ...
   LEzGP_data=train_QQ[rows,]#extract the matches from test data

#------------------------------------------------------------------------------------
   #model local points
    model = EzGP_fit(LEzGP_data[,1:(p+q)],LEzGP_data[,(p+q+1)],p=p,q=q,m=m,tau=tau) 
