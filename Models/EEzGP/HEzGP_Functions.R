...
cov_HEzGP <- function(W1,W2, p, q, m, parv, tau = 0) {
  # W1 : matrix n1 x (p+q) 
  # W2 : matrix n2 x  (p+q) 
  npar = 1 + q + 2*p
  ## a help function
...
#2
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#EEzGP_fit_partioned  model to save current covariance
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
##modeling
HEzGP_fit_partioned <- function(X, Y, p, q, m, tau = 0, lb = "T", ub = "T",  x0 = "T",darg_est=NULL,
  ...
  eval_f_list <- function(parv) {
    ...
    #covariance matrix
    covm = cov_HEzGP(W1=X,W2=X, p=p, q=q, m=m, parv=parv, tau = tau)
  ...
  #Phi and Phi_inv
  covm <- cov_Hm(X, p, q, m, n, solpar, tau,models =2)
  y = as.matrix(Y)
  Tm = try(chol(covm), silent=TRUE)
  #round(t(Tm)%*%Tm,2) == round(covm,2)
  if ('try-error' %in% class(Tm)) {
...
#find ALC 
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
update_HEzGP_alc<- function(xm1, Xn, X,Y,n,parv,covm,invc,m){
  npar = 1 + q + p + p*sum(m)
  if(is.null(dim(xm1))) xm1 <- matrix(xm1, nrow = 1)
  #X_n+1 X_n+1
  Km1m1=cov_HEzGP(W1=xm1,W2=xm1, p=p, q=q, m=m, parv=parv, tau = tau)
  
  #X_n+1 and Xn
  K_m.m1=cov_HEzGP(W1=X,W2=xm1, p=p, q=q, m=m, parv=parv, tau = tau)
  
  #Xref and  X
  Kmn=cov_HEzGP(W1=X,W2=Xn, p=p, q=q, m=m, parv=parv, tau = tau)
  
  #Xref X_n+1
  Km1n=cov_HEzGP(W1=xm1,W2=Xn, p=p, q=q, m=m, parv=parv, tau = tau)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
update_HEzGP_adaptive=function(xm1,La_train,X,Y,parv,covm,invc,n,m){
  npar = 1 + q + p + p
  if(is.null(dim(xm1))) xm1 <- matrix(xm1, nrow = 1)
  #X_n+1 X_n+1
  Km1m1=cov_HEzGP(W1=xm1,W2=xm1, p=p, q=q, m=m, parv=parv, tau = tau)
...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
update_HEzGP_loglike<- function(data, model, tau = 0, lb = "T", ub = "T",  x0 = "T",
                         xtol_rel = 1.0e-5, maxeval = 100, algorithm = "NLOPT_LD_LBFGS"){
  ...
  eval_f_list <- function(parv) {
    ...
    if(is.null(dim(x_ll))) x_ll <- matrix(x_ll, nrow = 1)
    #A=Phi_gg
    Phi_gg=cov_HEzGP(W1=X,W2=X, p=p, q=q, m=m, parv=parv, tau = tau)
    #B=Phi_gl
    Phi_gl=cov_HEzGP(W1=X,W2=x_ll, p=p, q=q, m=m, parv=parv, tau = tau)
    #D=Phi_ll
    Phi_ll=cov_HEzGP(W1=x_ll,W2=x_ll, p=p, q=q, m=m, parv=parv, tau = tau)
    ...
    grad_cor <- function(s) {
      ...
      der_m = matrix(0,n,n)
      Rtemp_cor <- apply(cbind(Xm1[rcoord[, 1], ], Xm1[rcoord[, 2], ]), 1, FUN = gradf_corhs, s=s)
      der_m[rcoord] <- Rtemp_cor
      der_m <- der_m + t(der_m)
      diag(der_m) = 0
      result = sum(diag(Phi_mm_inv %*% der_m)) - t(y-mu) %*% Phi_mm_inv %*% der_m %*% Phi_mm_inv %*% (y-mu)
