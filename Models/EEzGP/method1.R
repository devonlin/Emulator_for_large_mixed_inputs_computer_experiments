...
#global
start_time <- Sys.time()
model_gg = HEzGP_fit_partioned(twin1[,1:(p+q)],twin1[,(p+q+1)],p=p,q=q,m=m,tau=tau) 
end_time <- Sys.time()
...
parafunc_foreach=function(iu,twin1,p,q,m,tau,local_size,twin1_indices){
  ...
  twin_kd_tree=twin2[order(dst)[1:local_size],]
  #------------------------------------------------------------------------------------
  #update  the effect of l local points
  model_loglike_update=update_HEzGP_loglike(data=twin_kd_tree, model=model_gg, tau = tau)
  pred= HEzGP_predict(test_QQ[iu,1:(p+q)], model_loglike_update, MSE_on = 0)
...
results <- foreach(iu=icount(nr), .combine='rbind',
                   .packages=c('nloptr', "laGP","twinning")) %dopar% {
